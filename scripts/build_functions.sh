...
build_liblzma() {
	cmake_build "liblzma" "$BUILD_DIR/xz" "true"
}
...
build_openssl() {
	...
	case "$ARCH" in
    aarch64) openssl_target="mingw64-aarch64" ;;
    x86)     openssl_target="mingw" ;;
...
build_twolame() {
	...
	generate_pkgconfig twolame "TwoLAME MPEG Audio Layer II encoder" 0.4.0 "-ltwolame" "-I\${includedir}" "" "" "-DLIBTWOLAME_STATIC"
...
build_xvidcore() {
	...
	[ ! -f "$PREFIX/lib/libxvidcore.a" ] && [ -f "$PREFIX/lib/xvidcore.a" ] && mv "$PREFIX/lib/xvidcore.a" "$PREFIX/lib/libxvidcore.a"
...
build_kvazaar() {
	...
	autotools_build "kvazaar" "$BUILD_DIR/kvazaar" --disable-tools --disable-tests
}
...
generate_pkgconfig \
  ...
  "The GObject-based SVG rendering library" \
  "2.61.1" \
  "-L\${libdir} -lrsvg_2 -lcairo-gobject -lxml2 -lpangocairo-1.0 -lpixman-1 -lpng16 -lpcre2-8 -lffi -lexpat -lharfbuzz -lcairo -lgobject-2.0 -lfreetype -lglib-2.0 -lz -lws2_32 -lwinmm" \
  "-I\${includedir}/librsvg-2.0 -I\${includedir}/librsvg"
...
build_flite() {
   ...
   set_autotools_env
    ...
    ./configure "${COMMON_AUTOTOOLS_FLAGS[@]}" --with-audio=none
    
    make -j$(nproc)

    cd $BUILD_DIR/flite/bin
for f in *.exe; do mv "$f" "${f%.exe}"; done
cd ../
...
build_highway() {
    ...
    case "$ARCH" in
        x86|x86_64)     arch_opts[1]="-Dsse2=true" ;;
    esac
...
build_cairo() {
    sed -i "s/-Wp,-D_FORTIFY_SOURCE=2/-Wp,-D_FORTIFY_SOURCE=0/g" "$BUILD_DIR/cairo/meson.build"
    meson_build "Cairo" "$BUILD_DIR/cairo" "$CROSS_FILE_TEMPLATE" \
        ...
        -Dgtk_doc=false \
        -Dc_args="-DCAIRO_WIN32_STATIC_BUILD -U_FORTIFY_SOURCE -D_FORTIFY_SOURCE=0" \
        -Dc_link_args="$LDFLAGS -lssp -lfreetype -lfontconfig -lpixman-1 -lbrotlidec -lbrotlicommon -lz"
}

build_pango() {
    rm -f "$BUILD_DIR/pango/glib.wrap"
    
    # Generate custom cross-file for Pango with brotli libs
    local PANGO_CROSS_FILE="${CROSS_FILE_TEMPLATE}.pango"
    local PANGO_LDFLAGS="$LDFLAGS -lbrotlicommon"
    
    local S_CFLAGS=$(sanitize_flags "$CFLAGS")
    local S_CXXFLAGS=$(sanitize_flags "$CXXFLAGS")
    local S_LDFLAGS=$(sanitize_flags "$PANGO_LDFLAGS")
    
    cat >"$PANGO_CROSS_FILE" <<EOF
[binaries]
c = '$CC_ABS'
cpp = '$CXX_ABS'
ar = '$AR_ABS'
...
EOF
    ...
    meson_build "Pango" "$BUILD_DIR/pango" "$PANGO_CROSS_FILE" \
        ...
        -Dc_args="-DCAIRO_WIN32_STATIC_BUILD -D_FORTIFY_SOURCE=0"
}
...
build_bzip2() {
 rm -rf "$BUILD_DIR/bzip2/build" && mkdir -p "$BUILD_DIR/bzip2/build" && cd "$BUILD_DIR/bzip2/build"
    meson setup .. --cross-file="$CROSS_FILE_TEMPLATE" --prefix="$PREFIX" --buildtype=release --default-library=static -Ddocs=disabled
	ninja
	cp bzip2.exe bzip
...
build_vmaf() {
    meson_build "libvmaf" "$BUILD_DIR/vmaf/libvmaf" "$CROSS_FILE_TEMPLATE" \
	 -Denable_tests=false \
     -Denable_docs=false
}
...
build_libcodec2_native() {
	...
	cmake -G Ninja "$BUILD_DIR/libcodec2" \
		-DCMAKE_BUILD_TYPE=Release \
...
build_lensfun() {
	...
	case "$ARCH" in
		x86|x86_64)
			cmake_args+=(-DBUILD_FOR_SSE=ON -DBUILD_FOR_SSE2=ON)
			;;
		*)
			cmake_args+=(-DBUILD_FOR_SSE=OFF -DBUILD_FOR_SSE2=OFF)
			;;
	esac
	cmake_build "lensfun" "$BUILD_DIR/lensfun" "true" \
...
build_openapv() {
    ...
    [[ "$ARCH" == "x86" || "$ARCH" == "x86_64" ]] && PASSED_FLAGS+=(-DUNIVERSAL=1)
...
build_chromaprint() {
	cmake_build "Chromaprint" "$BUILD_DIR/chromaprint" true \
		...
		-DBUILD_TESTS=OFF \
		-DFFT_LIB=fftw3 \
		-DCMAKE_C_FLAGS="$CFLAGS -DCHROMAPRINT_STATIC -msse4.1" \
        -DCMAKE_CXX_FLAGS="$CXXFLAGS -DCHROMAPRINT_STATIC -msse4.1"
}
...
build_libmodplug() {
    ...
    generate_pkgconfig "libmodplug" \
        "MODPlug library for module audio files" \
        "0.8.9.0" \
        "$PREFIX/lib/libmodplug.a" \
        "-I\${includedir} -DMODPLUG_STATIC" \
        "" \
        ""
}

build_libmodplug_autotools() {
    local build_dir="$BUILD_DIR/libmodplug"
    ...
    [ -f configure ] || autoreconf -fi
    CFLAGS="$CFLAGS -I$PREFIX/include -DMODPLUG_STATIC" \
    CXXFLAGS="$CXXFLAGS -I$PREFIX/include -DMODPLUG_STATIC" \
...
build_libklvanc () {
sed -i '/subdir([[:space:]]*'\''tools'\'')/ s/^/#/' "$BUILD_DIR/libklvanc/meson.build"
